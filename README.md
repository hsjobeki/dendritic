...
A [Nix](https://nix.dev) [module-system](https://nix.dev/tutorials/module-system/deep-dive.html) usage pattern
...
The dendritic pattern leverages **deferred modules**—modules that can be instantiated multiple times with different configurations. This utilizes the Nixpkgs module system's ability to delay module evaluation until all configuration is available.
...
**Understanding deferred modules is essential before adopting this pattern.** The pattern is about architectural principles as opposed to implementation details. As such, flake-parts is not strictly required — it simply offers more ergonomic integration with flakes. You can implement the dendritic pattern using plain `lib.evalModules`.
